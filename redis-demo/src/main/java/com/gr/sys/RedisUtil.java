...
public class RedisUtil {
    ...
    static {
        try {
            ...
            config.setMaxTotal(Integer.valueOf(props.getProperty("spring.redis.pool.max-active")));
            //控制一个pool最多有多少个状态为idle(空闲的)的jedis实例，默认值8。
            config.setMaxIdle(Integer.valueOf(props.getProperty("spring.redis.pool.max-idle")));
            //等待可用连接的最大时间，单位毫秒，默认值为-1，表示永不超时。如果超过等待时间，则直接抛出JedisConnectionException；
            config.setMaxWaitMillis(Long.valueOf(props.getProperty("spring.redis.pool.max-wait")));
            //在borrow一个jedis实例时，是否提前进行validate操作；如果为true，则得到的jedis实例均是可用的；
            config.setTestOnBorrow(true);

            String ADDR = props.getProperty("spring.redis.host");
            Integer PORT = Integer.valueOf(props.getProperty("spring.redis.port"));
            Integer TIMEOUT = Integer.valueOf(props.getProperty("spring.redis.timeout"));
            String AUTH = props.getProperty("spring.redis.password");
    ...
    /**
     * <p>通过key从list的头部删除一个value,并返回该value,注意，此方法是阻塞的，并有超时时间</p>
     * @param timeout
     * @param key
     * @return 0:key 1:value1, 2:value2 ...
     */
